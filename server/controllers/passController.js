...
const { changePassEmail } = require("../emails/changePass");
  ...
  try {
    connection.query(
      ...
      async (error, results, fields) => {
        ...
        const link = `${process.env.SERVER_URL}/api/v1/reset?id=${user.user_id}&token=${token}`;
        console.log(link);

        const content = changePassEmail
            .replace("{name}", user.username)
            .replace("{link}", link)
        const subject = "Password Resest Request"
        ...
        sendEmail(content, subject, userEmail)
              .then((results) => {
                ...
                return res.status(201).json({
                  message:
                    "Email sent successfully. Kindly check your inbox for details",
                });
              })
              .catch((err) => {
                console.error(err);
